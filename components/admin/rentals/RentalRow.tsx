...
export default function RentalRow({rental}: { rental: Rental }) {
  ...
  return <TableRow key={rental.id}>
    <TableCell className="px-4 py-2">{rental.user.username}</TableCell>
    ...
    <TableCell className="px-4 py-2">
      {!rental.returned ? <Button onClick={handleReturn}>반납처리</Button> : null}
    </TableCell>
  </TableRow>;
}